...
public class SearchResultMerger {
  ...
  public List<BookSearchItem> merge(
      final List<Book> localBooks, final List<BookSearchItem> externalBooks) {
  ...
  private BookSearchItem toEphemeralExternalItem(final BookSearchItem external) {
    final String normalizedSourceReference =
        BookNormalization.normalizeSourceReference(external.sourceReference());
    final String ephemeralId =
        normalizedSourceReference != null ? EXTERNAL_PREFIX + normalizedSourceReference : "unknown";
    return new BookSearchItem(
        ephemeralId,
        external.title(),
        external.authorName(),
