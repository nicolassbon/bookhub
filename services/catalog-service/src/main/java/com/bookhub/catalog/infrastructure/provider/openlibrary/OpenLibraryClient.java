...
public class OpenLibraryClient implements SearchProvider {
  ...
  public List<BookSearchItem> search(final String query, final int limit) {
    try {
      final OpenLibrarySearchResponse response =
          openLibraryCircuitBreaker.executeSupplier(
              () ->
                  openLibraryRestClient
                      .get()
                      .uri(
                          uriBuilder ->
                              uriBuilder
      ...
      return response.docs().stream().map(this::toSearchItem).toList();
    } catch (CallNotPermittedException exception) {
      meterRegistry
          .counter("catalog.provider.openlibrary.search.fallbacks", "reason", "circuit_open")
          .increment();
      log.warn("OpenLibrary search short-circuited by circuit breaker. query='{}'", query);
      return List.of();
    } catch (RestClientException exception) {
      meterRegistry
          .counter("catalog.provider.openlibrary.search.fallbacks", "reason", "error")
          .increment();
      return List.of();
    }
  }
    ...
    return CompletableFuture.supplyAsync(() -> search(query, limit), executor)
        ...
        .exceptionally(
            throwable -> {
              ...
              meterRegistry
                  .counter("catalog.provider.openlibrary.search.fallbacks", "reason", reason)
                  .increment();
              log.warn(
                  "OpenLibrary async search fallback triggered. query='{}', reason='{}'",
                  query,
                  reason);
              return List.of();
            });
  ...
  public Book fetchDetail(final String sourceReference) {
    try {
      ...
      final OpenLibraryWorkResponse response =
          openLibraryCircuitBreaker.executeSupplier(
              () ->
                  openLibraryRestClient
                      .get()
                      .uri(uriBuilder -> uriBuilder.path("/works/{workId}.json").build(normalized))
                      .retrieve()
                      .body(OpenLibraryWorkResponse.class));

      if (response == null) {
        throw new ExternalProviderException(
            "OpenLibrary detail not found", new RestClientException("Empty response"));
      }
      ...
      throw new ExternalServiceUnavailableException(
          "OpenLibrary detail is temporarily unavailable", exception, retryAfterSeconds());
    } catch (RestClientException exception) {
      throw new ExternalServiceUnavailableException(
          "OpenLibrary detail fetch failed", exception, retryAfterSeconds());
    }
  }

  private int retryAfterSeconds() {
    return Math.max(
        1, (int) (openLibraryProperties.circuitBreaker().waitDurationOpenStateMs() / 1000));
  }
