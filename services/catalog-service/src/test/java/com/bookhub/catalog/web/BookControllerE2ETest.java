...
class BookControllerE2ETest {
    ...
    @Autowired
    private CircuitBreaker openLibraryCircuitBreaker;
    ...
    static void configureProperties(final DynamicPropertyRegistry registry) {
        ...
        registry.add("catalog.providers.openlibrary.circuit-breaker.minimum-number-of-calls",
                () -> 2);
        registry.add("catalog.providers.openlibrary.circuit-breaker.sliding-window-size", () -> 2);
        registry.add("catalog.providers.openlibrary.circuit-breaker.failure-rate-threshold",
                () -> 50);
        registry.add("catalog.providers.openlibrary.circuit-breaker.wait-duration-open-state-ms",
                () -> 30000);
    }
    ...
    void shouldMergeSearchResultsAndPersistExternalBookOnDetailFetch() throws Exception {
        final Book localBook = bookRepository
                .save(Book.builder().id(UUID.fromString("123e4567-e89b-12d3-a456-426614174000"))
                        .title("The Hobbit").authorName("J.R.R. Tolkien").sourceReference("OL123W")
                        .isbn13("9780261103344").build());

        WIRE_MOCK_SERVER.stubFor(WireMock.get(urlPathEqualTo("/search.json")).willReturn(okJson("""
                {
                  "docs": [
                    {
                      "key": "/works/OL123W",
        ...
        mockMvc.perform(get("/api/v1/books").queryParam("q", "tolkien")).andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(localBook.getId().toString()))
                .andExpect(jsonPath("$[1].id").value("ext:ol:OL999W"));

        WIRE_MOCK_SERVER
                .stubFor(WireMock.get(urlPathMatching("/works/OL999W.json")).willReturn(okJson("""
                        {
                          "key": "/works/OL999W",
                          "title": "Unfinished Tales",
                          "covers": [222]
        ...
        mockMvc.perform(get("/api/v1/books/{id}", "ext:ol:OL999W")).andExpect(status().isOk())
    ...
    void shouldReturnInvalidProviderPayloadWhenDetailTitleIsMissing() throws Exception {
        WIRE_MOCK_SERVER.stubFor(
                WireMock.get(urlPathMatching("/works/OLMISSINGW.json")).willReturn(okJson("""
                        {
    ...
    void shouldReturnDegraded503AndShortCircuitWhenBreakerIsOpen() throws Exception {
        ...
        WIRE_MOCK_SERVER.verify(exactly(2),
                WireMock.getRequestedFor(urlPathMatching("/works/OLOUTAGEW.json")));
    }
}
