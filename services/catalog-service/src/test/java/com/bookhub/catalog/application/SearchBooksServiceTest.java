...
class SearchBooksServiceTest {
  ...
  void setUp() {
    ...
    searchBooksService =
        new SearchBooksService(
            bookRepository, searchProvider, Duration.ofMillis(100), directExecutor, meterRegistry);
  }
  ...
  void shouldMergeLocalAndExternalResultsConcurrently() {
    ...
    final BookSearchItem external =
        new BookSearchItem(
            "OL262758W", "The Hobbit", "J.R.R. Tolkien", "OL262758W", "9780261103344", null, null);

    when(bookRepository.searchByQuery("hobbit", 20)).thenReturn(List.of(localBook));
  ...
  void shouldExposeEphemeralIdForUnpersistedExternalBooks() {
    ...
    when(bookRepository.searchByQuery("silmarillion", 20)).thenReturn(List.of());
  ...
  void shouldRecordSearchDurationMetrics() {
    when(bookRepository.searchByQuery("hobbit", 20)).thenReturn(List.of());
    when(searchProvider.searchAsync(eq("hobbit"), eq(20), any()))
  ...
  void shouldUseCandidateLimitAndApplyOffsetLimitSlice() {
    ...
    final Book localTwo =
        Book.builder()
            ...
            .build();
    final BookSearchItem externalOne =
        new BookSearchItem("ext:ol:OL003W", "Book Three", "Author", "OL003W", null, null, null);
    final BookSearchItem externalTwo =
        new BookSearchItem("ext:ol:OL004W", "Book Four", "Author", "OL004W", null, null, null);
