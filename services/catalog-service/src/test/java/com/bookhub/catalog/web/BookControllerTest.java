...
class BookControllerTest {
  ...
  @MockitoBean private SearchBooksService searchBooksService;

  @MockitoBean private GetBookDetailService getBookDetailService;

  @Test
  void shouldRejectQueryShorterThanTwoCharacters() throws Exception {
    mockMvc
        .perform(get("/api/v1/books").queryParam("q", "a"))
  ...
  void shouldRejectQueryLongerThan200Characters() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/books").queryParam("q", longQuery))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("VALIDATION_ERROR"));
  }
  ...
  void shouldRejectLimitExceedingMaximum() throws Exception {
    mockMvc
        .perform(get("/api/v1/books").queryParam("q", "hobbit").queryParam("limit", "200"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("VALIDATION_ERROR"));
  }
  ...
  void shouldRejectNegativeOffset() throws Exception {
    mockMvc
        .perform(get("/api/v1/books").queryParam("q", "hobbit").queryParam("offset", "-1"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code").value("VALIDATION_ERROR"));
  }
  ...
  void shouldReturnBookDetail() throws Exception {
    ...
    when(getBookDetailService.getById(bookId.toString()))
        .thenReturn(GetBookDetailService.BookDetailResult.success(book));
  ...
  void shouldReturnNotFoundErrorShape() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/books/{id}", "ext:ol:OL404W"))
  ...
  void shouldReturnDegradedDetailWhenProviderIsUnavailableAndNoCacheExists() throws Exception {
    when(getBookDetailService.getById("ext:ol:OLOUTAGEW"))
        .thenReturn(
            GetBookDetailService.BookDetailResult.degraded(
                new GetBookDetailService.DegradedDetail(
                    "ext:ol:OLOUTAGEW",
  ...
  void shouldReturnStructuredNotFoundForUnknownRoute() throws Exception {
    mockMvc
        .perform(get("/api/v1/books/unknown/route"))
        .andExpect(status().isNotFound())
