...
class OpenLibraryPropertiesTest {
    ...
    void shouldBindCircuitBreakerPropertiesFromConfiguration() {
        final ConfigurationPropertySource source = new MapConfigurationPropertySource(Map.of(
                ...
                "catalog.providers.openlibrary.circuit-breaker.wait-duration-open-state-ms", "12000",
                "catalog.providers.openlibrary.circuit-breaker.permitted-number-of-calls-in-half-open-state", "3"));

        final OpenLibraryProperties properties = new Binder(source)
    ...
    void shouldApplyCircuitBreakerDefaultsWhenNotConfigured() {
        final OpenLibraryProperties properties = new OpenLibraryProperties(true, "https://openlibrary.org", 2000, null);
