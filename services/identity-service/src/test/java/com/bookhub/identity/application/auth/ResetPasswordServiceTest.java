...
class ResetPasswordServiceTest {
  ...
  void shouldResetPasswordWhenTokenConsumptionIsValid() {
    ...
    final User user =
        User.rehydrate(userId, "nico", "nico@example.com", "old-password", "Nico", UserRole.USER);
    when(clock.instant()).thenReturn(Instant.parse("2026-04-12T20:00:00Z"));
    when(passwordResetTokenHasher.hash("raw-token")).thenReturn("hashed-token");
    when(passwordResetTokenRepository.consumeUserIdByTokenHash(
            "hashed-token", Instant.parse("2026-04-12T20:00:00Z")))
        .thenReturn(java.util.Optional.of(userId));
    when(userRepository.findById(userId)).thenReturn(Optional.of(user));
    when(passwordEncoder.encode("NewStrongPassword123!")).thenReturn("encoded-new-password");
  ...
  void shouldRejectResetWhenTokenDoesNotExist() {
    when(passwordResetTokenHasher.hash("unknown-token")).thenReturn("unknown-token-hash");
    when(passwordResetTokenRepository.consumeUserIdByTokenHash(
            "unknown-token-hash", Instant.parse("2026-04-12T20:00:00Z")))
        .thenReturn(java.util.Optional.empty());
    when(clock.instant()).thenReturn(Instant.parse("2026-04-12T20:00:00Z"));

    assertThatThrownBy(() -> resetPasswordService.reset("unknown-token", "NewStrongPassword123!"))
  ...
  void shouldRejectResetWhenTokenConsumptionFails() {
    when(clock.instant()).thenReturn(Instant.parse("2026-04-12T20:00:00Z"));
    when(passwordResetTokenHasher.hash("expired-token")).thenReturn("expired-token-hash");
    when(passwordResetTokenRepository.consumeUserIdByTokenHash(
            "expired-token-hash", Instant.parse("2026-04-12T20:00:00Z")))
        .thenReturn(java.util.Optional.empty());

    assertThatThrownBy(() -> resetPasswordService.reset("expired-token", "NewStrongPassword123!"))
        .isInstanceOf(InvalidPasswordResetTokenException.class)
        .hasMessage("Invalid or expired password reset token");
  ...
  void shouldRejectResetWhenTokenUserDoesNotExist() {
    ...
    when(clock.instant()).thenReturn(Instant.parse("2026-04-12T20:00:00Z"));
    when(passwordResetTokenHasher.hash("valid-token")).thenReturn("valid-token-hash");
    when(passwordResetTokenRepository.consumeUserIdByTokenHash(
            "valid-token-hash", Instant.parse("2026-04-12T20:00:00Z")))
        .thenReturn(java.util.Optional.of(userId));
    when(userRepository.findById(userId)).thenReturn(Optional.empty());

    assertThatThrownBy(() -> resetPasswordService.reset("valid-token", "NewStrongPassword123!"))
        .isInstanceOf(InvalidPasswordResetTokenException.class)
        .hasMessage("Invalid or expired password reset token");
