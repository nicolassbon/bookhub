...
class RefreshIntegrationTest {
    ...
    void shouldRotateRefreshTokenAndReturnNewAccessToken() throws Exception {
        ...
        refreshTokenJpaRepository.save(AuthIntegrationFixture.refreshToken(oldToken, user, Instant.now().plusSeconds(3600)));

        final var response = mockMvc.perform(post("/api/v1/auth/refresh")
    ...
    void shouldReturn401WhenRefreshTokenIsExpired() throws Exception {
        ...
        refreshTokenJpaRepository.save(AuthIntegrationFixture.refreshToken(expiredToken, user, Instant.now().minusSeconds(120)));
