...
class PostgreSqlIntegrationHardeningTest {
  ...
  void shouldNotSilentlySkipPostgreSqlIntegrationTestsWhenDockerIsUnavailable() {
    final Testcontainers annotation =
        PostgreSqlIntegrationTest.class.getAnnotation(Testcontainers.class);
    ...
    assertThat(annotation.disabledWithoutDocker())
        .as(
            "PostgreSqlIntegrationTest must fail fast instead of skipping when Docker is unavailable")
        .isFalse();
  }
  ...
  void shouldKeepTestProfileDatasourceFreeFromH2FallbackDefaults() throws IOException {
    final var configResource =
        getClass().getClassLoader().getResourceAsStream("application-test.yml");
