...
public class GetOwnProfileService {
  ...
  public GetOwnProfileService(final UserRepository userRepository) {
    this.userRepository = userRepository;
  }

  public User execute(final String subject) {
    final UUID userId = UUID.fromString(subject);
    return userRepository.findById(userId).orElseThrow(() -> new UserNotFoundException(subject));
  }
}
